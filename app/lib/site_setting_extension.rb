...
module SiteSettingExtension
  ...
  # just like a setting, except that it is available in javascript
  ...
  def add_override!(name,val)
    ...
    if type == types[:enum]
      raise Mike::InvalidParameters.new(:value) unless enum_class(name).valid_value?(val)
    end
  ...
  def set(name, value)
    if has_setting?(name)
      ...
      Mike.request_refresh! if requires_refresh?(name)
    else
